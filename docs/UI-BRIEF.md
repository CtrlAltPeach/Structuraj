...
4. **Выбран массив** — панель значений пуста по смыслу: массивы значений не показывают, они есть только в структуре. Нужен внятный текст, а не пустой экран. Проверять надо `types` (все ли они `array`), а не флаг `isArray`: он поднимается по одному вхождению и врёт на ключах со смешанными типами.
